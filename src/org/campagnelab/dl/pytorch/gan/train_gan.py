...
if __name__ == '__main__':
    ...
    class _netNoisePredictor(nn.Module):
        def __init__(self, ngpu):
            super(_netNoisePredictor, self).__init__()
            self.ngpu = ngpu
            self.main = nn.Sequential(
                # input is (nc) x 64 x 64
                nn.Conv2d(nc, ndf, 4, 2, 1, bias=False),
    ...
    netNoisePredictor = _netNoisePredictor(ngpu)
    netNoisePredictor.apply(weights_init)

    print(netNoisePredictor)
    ...
    for epoch in range(opt.niter):
        for i, data in enumerate(dataloader, 0):
            ...
            # train with fake
            netNoisePredictor.zero_grad()
            optimizerNP.zero_grad()
